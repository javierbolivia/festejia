...
export async function publicarInvitacionAdmin(invitacionId) {
  const { data: inv, error: errInv } = await supabase
    .from('express_invitaciones')
    .select('id, slug, nombre1, nombre2, fecha_evento')
    .eq('id', invitacionId)
    .single()

  if (errInv || !inv) return { data: null, error: errInv || new Error('Invitación no encontrada') }

  let slugFinal = inv.slug
  if (!slugFinal) {
    const base = [inv.nombre1, inv.nombre2].filter(Boolean).join('-y-').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'invitacion'
    const { data: existente } = await supabase.from('express_invitaciones').select('id').eq('slug', base).neq('id', invitacionId).maybeSingle()
    slugFinal = existente ? `${base}-${Math.floor(Math.random() * 1000)}` : base
  }
