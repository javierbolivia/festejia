...
/**
 * Combina en UN SOLO UPDATE lo que antes eran dos escrituras secuenciales
 * (fijarSlugSiDisponible + marcarPendientePago), llamadas una tras otra
 * desde el editor. Corrección de auditoría (hallazgos 6.1 y 6.2):
 *
 * - 6.2: ambas funciones actualizaban la misma fila sin ninguna dependencia
 *   real entre sí que impidiera combinarlas — dos round-trips de red donde
 *   bastaba uno.
 * - 6.1: el editor descartaba el `error` de ambas llamadas (solo
 *   destructuraba `data`). Si la primera tenía éxito pero la segunda
 *   fallaba, la invitación quedaba con slug fijado pero SIN pasar a
 *   pendiente_pago, y el flujo de pago se disparaba igual — estado
 *   inconsistente e invisible. Con una sola escritura atómica, ese estado
 *   intermedio inconsistente ya no puede existir: o se guardan slug+estado
 *   juntos, o no se guarda nada, y el error se propaga a quien llama.
 */
export async function prepararParaPublicar(id, userId, slugBase) {
  return actualizarConSlugUnico(
    (campos) =>
      supabase
        .from('express_invitaciones')
        .update({ ...campos, estado: 'pendiente_pago' })
        .eq('id', id)
        .eq('user_id', userId)
        .select()
        .single(),
...
/**
 * Elimina cualquier invitación desde /admin-express, sin importar de qué
 * cliente sea (a diferencia de eliminarInvitacion(), que solo permite al
 * propio dueño borrar la suya). Requiere la política RLS de admin agregada
 * en scripts/express-migration-admin.sql.
 */
export async function eliminarInvitacionAdmin(invitacionId) {
  return supabase.from('express_invitaciones').delete().eq('id', invitacionId)
}
