...
export default function EditorEngine({ invitacion, userId, onInvitacionActualizada, onSolicitarPublicar, puedeEditar = true }) {
  ...
  // Corrección de auditoría (hallazgo 1.5): antes no había ningún cleanup
  // de timersRef. Si el usuario escribía y navegaba fuera del editor dentro
  // de la ventana de DEBOUNCE_MS (900ms), el setTimeout seguía vivo, hacía
  // la llamada de red a Supabase, y luego llamaba setEstadoGuardado /
  // onInvitacionActualizada sobre un componente ya desmontado.
  ...
  async function handleGenerarIA(tipo, campoKey) {
    ...
    try {
      // El endpoint ahora exige sesión (ver auditoría, hallazgo 5.1): sin el
      // token en el header Authorization, responde 401 y nunca llama a Kimi.
      ...
      if (res.status === 429) {
        setErrores([json?.error || 'Alcanzaste el límite de generaciones con IA por esta hora. Intenta más tarde.'])
      } else if (json?.texto) {
        setContenido((prev) => {
      ...
      // Si la IA falla por red, el usuario simplemente escribe el texto a mano.
    }
    setGenerandoCampo({})
  }
