...
// Metadatos visuales de cada plantilla disponible para el plan Express.
// Solo referencia miniaturas propias del módulo Express (no las Premium).
const PLANTILLAS_INFO = [
  { id: 'plantilla-a', nombre: 'Paradise', descripcion: 'Verde y dorado, estilo tropical elegante' },
  { id: 'plantilla-b', nombre: 'Clásica', descripcion: 'Blanco y dorado, minimalista' },
  { id: 'plantilla-c', nombre: 'Romance', descripcion: 'Rosa pastel, delicada y suave' },
  { id: 'plantilla-d', nombre: 'Jardín', descripcion: 'Verde natural, con flores' },
  { id: 'plantilla-e', nombre: 'Moderna', descripcion: 'Negro y dorado, contemporánea' },
]
  ...
  return (
    <ExpressDashboardLayout activeTab="nueva">
      ...
      <div className="express-plantillas-grid">
        {PLANTILLAS_INFO.filter((p) => PLANTILLAS_EXPRESS.includes(p.id)).map((p) => (
          <button
            ...
            disabled={creando}
          >
            <div className="express-plantilla-preview">
              <span>{p.nombre}</span>
            </div>
            <h3>{p.nombre}</h3>
            <p>{p.descripcion}</p>
      ...
      <style jsx global>{`
        ...
        .express-plantilla-preview { height: 140px; background: linear-gradient(135deg, #2d4a3e, #b8860b); display: flex; align-items: center; justify-content: center; color: white; font-family: 'Cormorant Garamond', serif; font-size: 1.4rem; }
